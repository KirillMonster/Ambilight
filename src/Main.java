...
public class Main {
    ...
    static float SATURATION = 0.0f; // Насыщенность цветов
    ...
    static void send_colours(String colours) throws InterruptedException {
        byte[] bytes = CODE_SERIAL.getBytes();
    ...
    static String processing_channel(int channel) {
        // Так как в ascii таблице наибольший код это - 127, то есть у нас получается разрешение одного цветового канала - 7 бит.
        ...
        if (channel >= 254) {
            // Наивысшее число канала мы можем получить это - 127 * 2 = 254, которое почти равняется 8 битному разрешению.
